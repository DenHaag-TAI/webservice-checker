		...
		<html>
			...
			<body>
				<xsl:for-each select="/xsd:schema">
					<h1>
						Voorschriften rapport van het berichten XSD : <xsl:value-of select="$filename" />
					</h1>
					...
					<table>
						...
						<tr>
							...
							<xsl:choose>
								<xsl:when test="fn:matches($filename,'[a-zA-Z0-9]+-messages-v\d+\.\d+\.xsd')">
									<td class="center"><xsl:call-template name="passed"/></td>
								</xsl:when>
								<xsl:otherwise>
									<td class="center"><xsl:call-template name="highpriority"/></td>
						...
						<tr>
							...
							<xsl:choose>
								<xsl:when test="fn:matches(./@targetNamespace,'http://contracts.denhaag.nl/([a-z0-9-]+/){1,}?v\d+') or fn:matches(./@targetNamespace,'http://messages.denhaag.nl/([a-z0-9-]+/){1,}?v\d+')">
									<td class="center"><xsl:call-template name="passed"/></td>
								</xsl:when>
								<xsl:otherwise>
									<td class="center"><xsl:call-template name="highpriority"/></td>
							...
							<td><h4>Locale berichten:</h4>http://contracts.denhaag.nl/{domein}/{applicatie}/v{majorversie}<h4>Globale berichten:</h4>http://messages.denhaag.nl/{domein}/{applicatie}/v{majorversie}<br/>http://messages.denhaag.nl/{domein}/v{majorversie}</td>
						</tr>
						<tr>
